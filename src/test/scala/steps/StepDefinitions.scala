...
class StepDefinitions extends ScalaDsl with EN {
  ...
  When("""^I get the square root of a positive following number$"""){ (numbers:DataTable) =>
    val list = numbers.asMaps(classOf[String], classOf[String])
    var i = 0
    while ( {
      i < list.size
    }) {
      result.append(calc.sqrt(list.get(i).get("firstNum").toDouble))
  ...
  When("""^I power the following numbers$"""){ (numbers:DataTable) =>
    ...
    while ( {
      ...
      result.append(calc.pow(list.get(i).get("Base").toInt, list.get(i).get("Exponent").toInt))
  ...
  When("""^I log a number by a number base$"""){ (numbers:DataTable) =>
    val list = numbers.asMaps(classOf[String], classOf[String])
    var i = 0
    while ( {
      i < list.size
    }) {
      result.append(calc.log(list.get(i).get("Number").toDouble, list.get(i).get("Base").toInt))
